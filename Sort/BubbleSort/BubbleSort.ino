...
void l_bubbleSort()
{
  ...
  for (j = 0; j < l_size - i - 1; j++)
    if (l_array[j] > l_array[j + 1])
    {
      tft.drawRect(l_scale * (j + 1), MAX_Y - l_array[j + 1], l_scale, l_array[j + 1], GREEN);
      tft.drawRect(l_scale * (j), MAX_Y - l_array[j], l_scale, l_array[j], GREEN);
      delay(1);
      tft.drawRect(l_scale * (j + 1), MAX_Y - l_array[j + 1], l_scale, l_array[j + 1], BLACK);
      tft.drawRect(l_scale * (j), MAX_Y - l_array[j], l_scale, l_array[j], BLACK);
      swap(l_array[j], l_array[j + 1]);
      tft.drawRect(l_scale * (j + 1), MAX_Y - l_array[j + 1], l_scale, l_array[j + 1], WHITE);
      tft.drawRect(l_scale * (j), MAX_Y - l_array[j], l_scale, l_array[j], WHITE);
      check_presses(100);
      while (!isRunning)
      {
        check_presses(30000);
